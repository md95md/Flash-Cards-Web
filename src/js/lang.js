export const translations = {
  en: {
    my_decks: 'My Decks',
    study: 'Study',
    stats: 'Progress',
    your_decks: 'Your Decks',
    add_deck: '+ New Deck',
    manage_cards: 'Manage Cards',
    delete_deck: 'Delete Deck',
    add_card: ' Add Card',
    question: 'Question',
    answer: 'Answer',
    cancel: 'Cancel',
    create: 'Create',
    save: 'Save',
    add: 'Add',
    select_deck: 'Select a deck to add or edit cards',
    choose_deck: 'Choose a deck to study',
    your_stats: 'Your progress',
    learned: 'learned',
    no_cards: 'No cards',
    deck_done: 'Deck complete!',
    deck_done_sub: 'Great! You went through all the cards.',
    back: '← Back',
    exit: '↓ exit',
    know: 'know →',
    dont_know: "← don't know",
    delete_confirm: 'Delete forever?',
    confirm: 'Confirm',
    create_deck: 'Create New Deck',
    deck_name: 'Deck name',
    deck_desc: 'Description',
    deck_name_limit: 'Deck name cannot exceed 20 characters.',
    deck_desc_limit: 'Description cannot exceed 60 characters.',
    card_field_limit: 'Cannot exceed 100 characters.',
    deck_desc_required: 'Please enter a description.',
    cards: 'cards',
    deck_complete_btn: 'Deck complete',
    cards_in_deck: 'Cards in deck:',
    learned_count: 'learned',
    cards_count: 'cards',
    no_cards_in_deck: 'No cards in this deck',
    add_card_btn: '+ Add Card',
    edit_card_btn: 'Edit Card',
    delete_card_btn: 'Delete Card',
    enter_question_answer: 'Please enter both question and answer',
    account: 'Account',
    accent_color: 'Interface color',
    // ─── Landing copy ───────────────────────────────
    hero_title: "POV: you're finally studying (for real)",
    hero_sub: 'Flash Card Game is a system that actually works',
    feature_create: 'Create decks',
    feature_study: 'Study & repeat',
    feature_track: 'Track progress',
    perk_no_sub: 'no subscription',
    perk_no_ads: 'no ads',
    perk_no_streak: 'no streak pressure',
    auth_title: "Let's try it out!",
    register: "Fine",
    // ────────────────────────────────────────────────
    welcome_back: 'Welcome back!',
    sign_in_sub: 'Pick up where you left off.',
    contact: 'Contact',
    copyright: '© 2026 Flash Card Game',
    password: 'passwrod:',
    email: 'email:',
    sign_in: 'Sign In',
    continue_google: 'Continue with Google',
    password_length: 'Password must be at least 6 characters',
    email_already_in_use: 'This email is already registered. Try signing in',
    something_went_wrong: 'Something went wrong. Please try again',
    or: 'or',
    enter_emailAndPassword: 'Please enter your email and password',
    incorrect_email_or_password: 'Incorrect email or password',
    valid_emailAndPassword: 'Please enter a valid email address',
    manage_deck: 'Manage deck',
    search_cards: 'Search',
    edit_deck_btn: 'Edit Deck',
    edit_deck_title: 'Edit Deck',
    import_json_btn: 'Import JSON',
    import_json_hint_title: 'JSON File Format',
    import_json_hint_desc: 'Your file must be a JSON array where each object has "question" and "answer" fields (or "front" / "back"):',
    import_json_hint_ai_label: '💡 Tip: ask an AI assistant to prepare the file for you. Copy the prompt below:',
    import_json_hint_prompt: 'I have a list of flashcards I want to import into a flashcard app. Please convert them into a JSON file in the following format:\n[\n  { "question": "...", "answer": "..." },\n  { "question": "...", "answer": "..." }\n]\nHere are my cards:\n[paste your cards here]',
    import_json_hint_copy: 'Copy prompt',
    import_json_hint_copied: 'Copied!',
    import_json_hint_proceed: 'Choose file',
    import_json_hint_skip: "Don't show this again",
    import_json_success: 'cards found',
    import_json_add_btn: 'Add to deck',
    import_json_invalid: 'Invalid JSON. Expected an array of objects with "question" and "answer" fields.',
    import_json_format_error: 'Invalid file format. Please review the format requirements below.',
    import_json_skipped: (n) => `${n} duplicate${n === 1 ? '' : 's'} found — ${n === 1 ? 'it' : 'they'} will not be added`,
    export_json_btn: 'Export JSON',
    export_json_title: 'cards to export',
    export_json_download_btn: 'Download',
    last_studied: 'last studied',
    session_learned: 'learned in session',
    not_known: 'not known',
    never: 'never',
    no_sessions: 'no study sessions yet',
    session_repeated: 'repeated',
    col_date: 'session date',
    col_learned: 'learned',
    col_not_known: 'not known',
    col_repeated: 'repeated',
    col_duration: 'duration',
    sessions_history: 'Sessions History',
    repeat: 'Repeat',
    card_limit_add: (current) => `Deck limit is 100 cards. This deck already has ${current} card${current === 1 ? '' : 's'}.`,
    card_limit_import: (adding, current) => `Deck limit is 100 cards. You're trying to add ${adding} cards to a deck with ${current} (${adding + current} total).`,
    deck_limit: `You've reached the limit of 10 decks. Delete an existing deck to create a new one.`,
  },
  ru: {
    my_decks: 'Мои колоды',
    study: 'Учёба',
    stats: 'Прогресс',
    your_decks: 'Ваши колоды',
    add_deck: '+ Новая колода',
    manage_cards: 'Управление карточками',
    delete_deck: 'Удалить колоду',
    add_card: 'Добавить карточку',
    question: 'Вопрос',
    answer: 'Ответ',
    cancel: 'Отмена',
    create: 'Создать',
    save: 'Сохранить',
    add: 'Добавить',
    select_deck: 'Выберите колоду, чтобы добавить или отредактировать карточки',
    choose_deck: 'Выберите колоду для учёбы',
    your_stats: 'Ваш прогресс',
    learned: 'выучено',
    no_cards: 'Нет карточек',
    deck_done: 'Колода завершена!',
    deck_done_sub: 'Отлично! Вы прошли все карточки.',
    back: '← Вернуться',
    exit: '↓ выход',
    know: 'знаю →',
    dont_know: '← не знаю',
    delete_confirm: 'Удалить навсегда?',
    confirm: 'Подтверждаю',
    create_deck: 'Создать новую колоду',
    deck_name: 'Название колоды',
    deck_desc: 'Описание',
    deck_name_limit: 'Название не может превышать 20 символов.',
    deck_desc_limit: 'Описание не может превышать 60 символов.',
    card_field_limit: 'Не может превышать 100 символов.',
    deck_desc_required: 'Пожалуйста, введите описание.',
    cards: 'карточек',
    deck_complete_btn: 'Колода выучена',
    cards_in_deck: 'Карточки в колоде:',
    learned_count: 'выучено',
    cards_count: 'карточек',
    no_cards_in_deck: 'В этой колоде нет карточек',
    add_card_btn: '+ Добавить карточку',
    edit_card_btn: 'Редактировать',
    delete_card_btn: 'Удалить карточку',
    enter_question_answer: 'Пожалуйста, введите и вопрос, и ответ',
    account: 'Аккаунт',
    accent_color: 'Цвет интерфейса',
    // ─── Landing copy ───────────────────────────────
    hero_title: 'POV: ты наконец учишься (на самом деле)',
    hero_sub: 'Flash Card Game — это система, которая реально работает',
    feature_create: 'Создавай колоды',
    feature_study: 'Учи и повторяй',
    feature_track: 'Следи за прогрессом',
    perk_no_sub: 'без подписки',
    perk_no_ads: 'без рекламы',
    perk_no_streak: 'без стрика',
    auth_title: 'Попробуем!',
    register: 'Ладно',
    // ────────────────────────────────────────────────
    welcome_back: 'С возвращением!',
    sign_in_sub: 'Продолжай с того места, где остановился.',
    contact: 'Контакты',
    copyright: '© 2026 Flash Card Game',
    password: 'пароль:',
    email: 'электронная почта:',
    sign_in: 'Войти',
    continue_google: 'Продолжить с Google',
    password_length: 'Пароль должен быть не менее 6 символов',
    email_already_in_use: 'Этот email уже зарегистрирован. Попробуйте войти',
    something_went_wrong: 'Что-то пошло не так. Пожалуйста, попробуйте снова',
    or: 'или',
    enter_emailAndPassword: 'Пожалуйста, введите ваш email и пароль',
    incorrect_email_or_password: 'Неверный email или пароль',
    valid_emailAndPassword: 'Пожалуйста, введите действительный адрес электронной почты',
    manage_deck: 'Управление колодой',
    search_cards: 'Поиск',
    edit_deck_btn: 'Редактировать',
    edit_deck_title: 'Редактировать колоду',
    import_json_btn: 'Импорт JSON',
    import_json_hint_title: 'Формат JSON-файла',
    import_json_hint_desc: 'Файл должен быть массивом JSON, где каждый объект содержит поля "question" и "answer" (или "front" / "back"):',
    import_json_hint_ai_label: '💡 Совет: попросите ИИ-ассистента подготовить файл за вас. Скопируйте промпт ниже:',
    import_json_hint_prompt: 'У меня есть список карточек для запоминания, которые я хочу импортировать в приложение. Пожалуйста, преобразуй их в JSON-файл в следующем формате:\n[\n  { "question": "...", "answer": "..." },\n  { "question": "...", "answer": "..." }\n]\nВот мои карточки:\n[вставь карточки сюда]',
    import_json_hint_copy: 'Копировать промпт',
    import_json_hint_copied: 'Скопировано!',
    import_json_hint_proceed: 'Выбрать файл',
    import_json_hint_skip: 'Не показывать снова',
    import_json_success: 'карточек найдено',
    import_json_add_btn: 'Добавить в колоду',
    import_json_invalid: 'Неверный JSON. Ожидается массив объектов с полями "question" и "answer".',
    import_json_format_error: 'Неверный формат загруженного файла. Ознакомьтесь с правилами формата ниже.',
    import_json_skipped: (n) => `${n} ${n === 1 ? 'дубликат' : n < 5 ? 'дубликата' : 'дубликатов'} уже есть в колоде — ${n === 1 ? 'он' : 'они'} не будут добавлены`,
    export_json_btn: 'Экспорт JSON',
    export_json_title: 'карточек для экспорта',
    export_json_download_btn: 'Скачать',
    last_studied: 'последнее повторение',
    session_learned: 'выучено за сеанс',
    not_known: 'не знаю',
    never: 'никогда',
    no_sessions: 'сеансов пока нет',
    session_repeated: 'повторено',
    col_date: 'дата сессии',
    col_learned: 'выучено',
    col_not_known: 'не знаю',
    col_repeated: 'повторено',
    col_duration: 'длительность',
    sessions_history: 'История сессий',
    repeat: 'Повторить',
    card_limit_add: (current) => `Лимит колоды — 100 карточек. В этой колоде уже ${current} карт${current === 1 ? 'очка' : current < 5 ? 'очки' : 'очек'}.`,
    card_limit_import: (adding, current) => `Лимит колоды — 100 карточек. Вы добавляете ${adding} к ${current} уже имеющимся (итого ${adding + current}).`,
    deck_limit: `Достигнут лимит в 10 колод. Удалите одну из существующих, чтобы создать новую.`,
  },
  es: {
    my_decks: 'Mis mazos',
    study: 'Estudiar',
    stats: 'Progreso',
    your_decks: 'Tus mazos',
    add_deck: '+ Nuevo mazo',
    manage_cards: 'Gestionar tarjetas',
    delete_deck: 'Eliminar mazo',
    add_card: 'Añadir tarjeta',
    question: 'Pregunta',
    answer: 'Respuesta',
    cancel: 'Cancelar',
    create: 'Crear',
    save: 'Guardar',
    add: 'Añadir',
    select_deck: 'Selecciona un mazo para añadir o editar tarjetas',
    choose_deck: 'Elige un mazo para estudiar',
    your_stats: 'Tus progreso',
    learned: 'aprendidas',
    no_cards: 'Sin tarjetas',
    deck_done: '¡Mazo completado!',
    deck_done_sub: '¡Genial! Has repasado todas las tarjetas.',
    back: '← Volver',
    exit: '↓ salir',
    know: 'sé →',
    dont_know: '← no sé',
    delete_confirm: '¿Eliminar para siempre?',
    confirm: 'Confirmar',
    create_deck: 'Crear nuevo mazo',
    deck_name: 'Nombre del mazo',
    deck_desc: 'Descripción',
    deck_name_limit: 'El nombre no puede superar los 20 caracteres.',
    deck_desc_limit: 'La descripción no puede superar los 60 caracteres.',
    card_field_limit: 'No puede superar los 100 caracteres.',
    deck_desc_required: 'Por favor, introduce una descripción.',
    cards: 'tarjetas',
    deck_complete_btn: 'Mazo aprendido',
    cards_in_deck: 'Tarjetas en el mazo:',
    learned_count: 'aprendidas',
    cards_count: 'tarjetas',
    no_cards_in_deck: 'No hay tarjetas en este mazo',
    add_card_btn: '+ Añadir tarjeta',
    edit_card_btn: 'Editar tarjeta',
    delete_card_btn: 'Eliminar tarjeta',
    enter_question_answer: 'Por favor, ingresa tanto la pregunta como la respuesta',
    account: 'Cuenta',
    accent_color: 'Color de interfaz',
    // ─── Landing copy ───────────────────────────────
    hero_title: 'POV: por fin estás estudiando (en serio)',
    hero_sub: 'Flash Card Game es un sistema que realmente funciona',
    feature_create: 'Crear mazos',
    feature_study: 'Estudiar y repetir',
    feature_track: 'Seguir progreso',
    perk_no_sub: 'sin suscripción',
    perk_no_ads: 'sin anuncios',
    perk_no_streak: 'sin racha',
    auth_title: '¡Vamos a probarlo!',
    register: 'Bueno',
    // ────────────────────────────────────────────────
    welcome_back: '¡Bienvenido de vuelta!',
    sign_in_sub: 'Continúa donde lo dejaste.',
    contact: 'Contacto',
    copyright: '© 2026 Flash Card Game',
    password: 'contraseña:',
    email: 'dirección de correo electrónico:',
    sign_in: 'Iniciar sesión',
    continue_google: 'Continuar con Google',
    password_length: 'La contraseña debe tener al menos 6 caracteres',
    email_already_in_use: 'Este correo ya está registrado. Intenta iniciar sesión',
    something_went_wrong: 'Algo salió mal. Por favor, intenta de nuevo',
    or: 'o',
    enter_emailAndPassword: 'Por favor, ingresa tu correo electrónico y contraseña',
    valid_emailAndPassword: 'Por favor, ingresa una dirección de correo electrónico válida',
    incorrect_email_or_password: 'Correo electrónico o contraseña incorrectos',
    manage_deck: 'Administrar mazo',
    search_cards: 'Buscar',
    edit_deck_btn: 'Editar mazo',
    edit_deck_title: 'Editar mazo',
    import_json_btn: 'Importar JSON',
    import_json_hint_title: 'Formato del archivo JSON',
    import_json_hint_desc: 'El archivo debe ser un array JSON donde cada objeto tenga los campos "question" y "answer" (o "front" / "back"):',
    import_json_hint_ai_label: '💡 Consejo: pídele a un asistente de IA que prepare el archivo por ti. Copia el prompt de abajo:',
    import_json_hint_prompt: 'Tengo una lista de tarjetas de memoria que quiero importar a una aplicación. Por favor, conviértelas a un archivo JSON con el siguiente formato:\n[\n  { "question": "...", "answer": "..." },\n  { "question": "...", "answer": "..." }\n]\nAquí están mis tarjetas:\n[pega tus tarjetas aquí]',
    import_json_hint_copy: 'Copiar prompt',
    import_json_hint_copied: '¡Copiado!',
    import_json_hint_proceed: 'Elegir archivo',
    import_json_hint_skip: 'No mostrar de nuevo',
    import_json_success: 'tarjetas encontradas',
    import_json_add_btn: 'Añadir al mazo',
    import_json_invalid: 'JSON inválido. Se espera un array de objetos con campos "question" y "answer".',
    import_json_format_error: 'Formato de archivo no válido. Por favor, revisa los requisitos de formato a continuación.',
    import_json_skipped: (n) => `${n} duplicado${n === 1 ? '' : 's'} encontrado${n === 1 ? '' : 's'} — no ${n === 1 ? 'será añadido' : 'serán añadidos'}`,
    export_json_btn: 'Exportar JSON',
    export_json_title: 'tarjetas para exportar',
    export_json_download_btn: 'Descargar',
    last_studied: 'último repaso',
    session_learned: 'aprendidas en sesión',
    not_known: 'no sé',
    never: 'nunca',
    no_sessions: 'aún no hay sesiones',
    session_repeated: 'repetidas',
    col_date: 'fecha de sesión',
    col_learned: 'aprendidas',
    col_not_known: 'no sé',
    col_repeated: 'repetidas',
    col_duration: 'duración',
    sessions_history: 'Historial de sesiones',
    repeat: 'Repetir',
    card_limit_add: (current) => `El límite del mazo es 100 tarjetas. Este mazo ya tiene ${current} tarjeta${current === 1 ? '' : 's'}.`,
    card_limit_import: (adding, current) => `El límite del mazo es 100 tarjetas. Intentas añadir ${adding} tarjetas a un mazo con ${current} (${adding + current} en total).`,
    deck_limit: `Has alcanzado el límite de 10 mazos. Elimina uno existente para poder crear uno nuevo.`,
  }
};
